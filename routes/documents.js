...
router.post('/', async function (req, res, next) {
  ...
  try {
  	...
  	if(!method) throw new Error("REQUEST DON'T HAVE METHOD")
  	switch(method.toUpperCase()) {
        ...
        let documentsInfo = await dbDocuments.query(`SELECT documents->'$[0].path' as path,
         documents->'$[0].company_id' as company_id, 
         documents->'$[0].factory_id' as factory_id, date, order_id 
          FROM documents, orders
         WHERE documents.order_id = orders.id ORDER BY order_id DESC;`)
