function pageTailsType(par) {
	...
	POST(`/${par}`, 'method=GET', function(res, status) {
		...
		if (res.status==200) {
			...
			createTailsTypeAddModal(par);

			let dataList = '';

			for (var i = 0; i < res.tails_types.length; i++) {	
				let attr = '';
				if (res.tails_types[i].attributes) {
					let obj = res.tails_types[i].attributes[0];
				...
				dataList += `<div class="table__row">
								<div class="table__cell" style="width: 20%;">${res.tails_types[i].name}</div>
								<div class="table__cell" style="width: 35%;">${res.tails_types[i].description}</div>
			...
			table.innerHTML =`<div class="table">
								<div class="table__row table__row-header">
									...
									<div class="table__cell" style="width: 35%;">Атрибуты</div>
...
function createTailsTypeAddModal(par) {
