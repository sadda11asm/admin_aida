function pageUploadType(par) {
	POST(`/${par}`, 'method=GET', function(res, status) {
		...
		createUploadTypeAddModal(par)	

		let dataList = '';

		for (var i = 0; i < res.attributes.length; i++) {
			dataList += `<div class="table__row">
							<div class="table__cell" style="width: 25%;">${res.attributes[i].name}</div>
							<div class="table__cell" style="width: 20%;">${res.attributes[i].description}</div>
							<div class="table__cell" style="width: 20%;">${res.attributes[i].attributes.width}</div>
							<div class="table__cell" style="width: 20%;">${res.attributes[i].attributes.length}</div>
		...
		table.innerHTML =`<div class="table">
							<div class="table__row table__row-header">
								<div class="table__cell" style="width: 25%;">Названия</div>
								<div class="table__cell" style="width: 20%;">Описания</div>
								<div class="table__cell" style="width: 20%;">Ширина</div>
								<div class="table__cell" style="width: 20%;">Длина</div>
...
function createUploadTypeAddModal(par) {
